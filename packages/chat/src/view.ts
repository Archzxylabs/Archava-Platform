...
/** An action the gate let through, and which ran. */
...
const GAP_MESSAGE =
  "No information was found for this request in the tenant’s sources.";
...
function truthBlocks(outcome: TurnOutcome): readonly ChatBlock[] {
  ...
  for (const subject of Object.keys(outcome.structuredTruth)) {
    truncated =
      flatten(subject, outcome.structuredTruth[subject], [], rows, 1) ||
      truncated;
  }
...
function flatten(
  ...
  if (isPlainRecord(value)) {
    ...
    for (const [key, child] of entries) {
      truncated =
        flatten(subject, child, [...prefix, key], rows, depth + 1) || truncated;
    }
    return truncated;
  }
...
function actionBlocks(outcome: TurnOutcome): readonly ChatBlock[] {
  ...
  for (const action of outcome.actions) {
    ...
    if (action.decision === "confirmation_required") {
      blocks.push({
        kind: "confirmation",
        actionId: action.actionId,
        reason: action.reason,
...
function inspectorBlocks(
  ...
  const withheldCtas = outcome.components
    .filter(
      (component) =>
        component.kind === "cta" && !drawn.has(component.props.actionId),
    )
    .map((component) =>
      component.kind === "cta" ? component.props.actionId : "",
    )
