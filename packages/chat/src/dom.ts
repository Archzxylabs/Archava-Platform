...
export interface ChatNodeLike {
  ...
  addEventListener(
    type: string,
    listener: (event: ChatEventLike) => void,
  ): void;
  removeEventListener(
    type: string,
    listener: (event: ChatEventLike) => void,
...
/** A click, as far as the shell looks. */
...
export function mount(
  ...
  if (doc === null || typeof doc.createElement !== "function") {
    throw new ChatEnvironmentError(
      "No document is available; the chat shell requires a DOM.",
    );
  }
  ...
  switch (block.kind) {
    ...
    case "sources":
      appendTitles(
        ...
        node,
        "archava-sources",
        block.citations.map(citationTitle),
      );
      return;
...
function appendBanner(
  ...
  const banner = element(builder, "p", [
    "archava-banner",
    `archava-banner-${tone}`,
  ]);
