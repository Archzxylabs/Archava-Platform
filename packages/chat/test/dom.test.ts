...
import type { GenerativeComponent, HandoffContext, TurnOutcome } from '@archava/assistant'
import {
  mount,
...
class FakeNode implements ChatNodeLike {
  ...
  removeChild(child: ChatNodeLike): ChatNodeLike {
    ...
    if (index < 0) {
      throw new Error(`removeChild: ${child.tagName} is not a child of ${this.tagName}`)
    }
  ...
  replaceChild(next: ChatNodeLike, previous: ChatNodeLike): ChatNodeLike {
    ...
    if (index < 0) {
      throw new Error(`replaceChild: ${previous.tagName} is not a child of ${this.tagName}`)
    }
...
class FakeHost implements ChatNodeLike {
  ...
  attachShadow(_options?: { readonly mode?: 'open' | 'closed' }): FakeNode | null {
...
function classes(node: ChatNodeLike): readonly string[] {
  return (node.getAttribute('class') ?? '').split(' ').filter((name) => name.length > 0)
}
...
function fire(node: FakeNode, type = 'click', key?: string): void {
  for (const listener of [...(node.listeners.get(type) ?? [])]) {
    listener(key === undefined ? { type } : { type, key })
  }
}
...
describe('mount (§24)', () => {
  it('opens a shadow root and puts the stylesheet inside it', () => {
    ...
    expect(find(harnessRoot, 'archava-sheet')?.textContent).toContain('.archava-log')
...
describe('show', () => {
  it('draws one message per turn, keyed by session and time', () => {
    ...
    expect(messages[0]?.getAttribute('data-key')).toBe(`${SESSION}:2026-04-01T09:00:00.000Z`)
    expect(messages[0]?.getAttribute('data-time')).toBe('2026-04-01T09:00:00.000Z')
  })
  ...
  it('re-draws the same turn in place rather than duplicating it', () => {
    ...
    expect(find(harnessRoot, 'archava-text')?.textContent).toBe('Room 14 is free.')
  ...
  it('keeps the log bounded, dropping the oldest message first', () => {
    ...
    for (let minute = 0; minute < 55; minute += 1) {
      handle.show(outcome({ occurredAt: minuteAt(minute), text: `Answer ${minute}` }))
    }
    const log = find(harnessRoot, 'archava-log')
    expect(log?.children).toHaveLength(50)
    expect(log?.children[0]?.getAttribute('data-key')).toBe(`${SESSION}:${minuteAt(5)}`)
  })
...
describe('clear and destroy', () => {
  it('clear empties the log and forgets the turns it drew', () => {
    ...
    handle.show(outcome())
    expect((find(harnessRoot, 'archava-log')?.children ?? []).length).toBeGreaterThan(0)
  })
...
describe('text is text', () => {
  ...
  it('writes a tenant-owned citation as a title, never as a link', () => {
    ...
    expect(find(harnessRoot, 'archava-sources')?.textContent).toBe('Cancellation policy')
  })
})
...
describe('events (§18)', () => {
  it('emits a submitted message, trimmed, and clears the field', () => {
    ...
    fire(send, 'click')
    expect(events).toEqual([{ kind: 'message_submitted', text: 'Is Room 12 free?' }])
    expect(input.value).toBe('')
  })

  it('emits nothing for an empty message', () => {
    const { events, root: harnessRoot } = harness()
    fire(requireNode(find(harnessRoot, 'archava-composer-send')), 'click')
    expect(events).toEqual([])
  })

  // A text field that answers only its button is not a field a visitor types in,
  // and this page is meant to be used the way a visitor would use it. Both paths
  // are pinned together because they must submit the same one event: a button
  // and a keypress that disagree would be two gates, one of them unguarded.
  it('submits on Enter, by the same one event path the button takes', () => {
    const { events, root: harnessRoot } = harness()
    const input = requireNode(find(harnessRoot, 'archava-composer-input'))
    input.value = '  compare the garden twin room and the treetop suite  '
    fire(input, 'keydown', 'Enter')
    expect(events).toEqual([
      { kind: 'message_submitted', text: 'compare the garden twin room and the treetop suite' },
    ])
    expect(input.value).toBe('')
  })

  it('submits on nothing but Enter, so a keystroke is not a turn', () => {
    const { events, root: harnessRoot } = harness()
    const input = requireNode(find(harnessRoot, 'archava-composer-input'))
    input.value = 'half a sentence'
    for (const key of ['a', 'Shift', 'Escape', 'Tab']) {
  ...
  it('emits an action request for a CTA, and never runs it', () => {
    ...
    handle.show(outcome({ components: [CTA], permittedActionIds: ['booking.create'] }))
    fire(requireNode(find(harnessRoot, 'archava-cta')))
    expect(events).toEqual([
      {
  ...
  it('re-emits the gate’s own inputs on a confirmation, unchanged', () => {
    ...
    fire(requireNode(card === null ? null : find(card, 'archava-confirmation-confirm')))
    expect(events).toEqual([
      {
        kind: 'action_requested',
        actionId: 'booking.create',
  ...
  it('reports a declined action as a decline, not as a request', () => {
    ...
    fire(requireNode(find(harnessRoot, 'archava-confirmation-decline')))
    expect(events).toEqual([{ kind: 'action_declined', actionId: 'booking.create' }])
  })

  it('offers the way back from a refusal, and asks with the gate’s own words', () => {
    // The refusal card is drawn, so the visitor who changes their mind has
    // somewhere to click. What the click emits is a *request* — the page re-runs
    // the turn with the action confirmed, and the gate decides again — and it
    // re-emits the inputs unchanged, exactly as the confirmation card does, so
    // undoing a refusal cannot become a different booking than the one refused.
    const { handle, events, root: harnessRoot } = harness()
    handle.show(
      outcome({
        actions: [
          {
            actionId: 'booking.create',
            policy: 'denied',
            denialReason: 'declined_by_visitor',
            execution: 'not_attempted',
            prompt: 'Book Room 12 for two nights?',
            reason: '"booking.create" was declined earlier in this session.',
            inputs: { nights: 2, roomId: 'room-12' },
          },
        ],
      }),
    )
    const card = requireNode(find(harnessRoot, 'archava-confirmation'))
    expect(card.getAttribute('data-archava-declined')).toBe('true')
    expect(card.getAttribute('data-action-id')).toBe('booking.create')
    fire(requireNode(find(card, 'archava-confirmation-confirm')))
    expect(events).toEqual([
      {
        kind: 'action_requested',
        actionId: 'booking.create',
  ...
  it('offers a person on a handoff card without escalating on its own', () => {
    ...
    expect(events).toEqual([])
    fire(button)
    expect(events).toEqual([{ kind: 'handoff_requested', label: 'Talk to a person' }])
  })
})
...
describe('a comparison frame (§17)', () => {
  ...
  it('leaves every cell empty, marked with the axes a host fills by', () => {
    ...
    expect(text(harnessRoot)).toContain('Values come from the tenant’s own systems.')
  })
})
