...
describe('trust surfaces (§17)', () => {
  ...
  it('shows no truth panel when the answer did not come from live data', () => {
    expect(kinds({ structuredTruth: { price: { amountMinor: 1 } } })).not.toContain('truth')
  })
})
...
describe('components (§25)', () => {
  ...
  it('withholds a CTA the gate did not permit', () => {
    expect(kinds({ components: [CTA], permittedActionIds: [] })).not.toContain('cta')
  })

  it('draws a CTA the gate permitted', () => {
    const block = only({ components: [CTA], permittedActionIds: ['booking.create'] }, 'cta')
...
describe('actions (§18)', () => {
  ...
  it('renders an action the executor reported as done', () => {
    const block = only(
      {
        actions: [
          {
  ...
  it('draws the refusal the visitor themselves made', () => {
    // The one denial a visitor can act on, because it is theirs. The gate's own
    // `denialReason` is what tells the two apart — every other denial is a fact
    // about the tenant, and drawing any of those as a decline card would let a
    // page the action was never on read as "you said no to this".
    const block = only(
      {
        actions: [
          {
    ...
    expect(block.actionId).toBe('booking.create')
    expect(block.reason).toContain('declined earlier in this session')
    // The prompt is carried, because the shell's "after all, do it" button asks
    // with it, and a card that dropped it would ask with a bare action id.
    expect(block.prompt).toBe('Book Room 12?')
    expect(block.inputs).toEqual({ nights: 2 })
  })
  it('does not draw a refusal as a pending question', () => {
    // A decline is answered, not open. Drawing it as a `confirmation` would put
    // the same yes/no question in front of the visitor a second time, which is
    // the re-ask loop the decline gate exists to stop.
    expect(
      kinds({
        actions: [
          {
            ...
            denialReason: 'declined_by_visitor',
            execution: 'not_attempted',
            reason: 'declined earlier in this session',
            inputs: {},
          },
        ],
      }),
  ...
  it('says a refusal is not an outcome', () => {
    expect(
      kinds({
        actions: [
          {
...
describe('handoff and inspector (§26, §16)', () => {
  ...
  it('shows everything the turn withheld when it was asked for', () => {
    ...
    expect(block.rejectedComponents).toEqual(['comparison_table: entityIds must have 2 items'])
...
describe('describeBlock', () => {
  it('names a source card by its title and excerpt', () => {
    ...
    expect(describeBlock(block)).toBe('Cancellation policy: Free until 48 hours.')
  })
  ...
  it('describes a refusal as the visitor’s own, and names the reason', () => {
    // The reason goes in the sentence because it is the guest-facing half: the
    // card is the one place the visitor learns their no is remembered, and a
    // bare "declined" would not say whether it can be undone.
    const block = only(
      {
        actions: [
          {
    ...
    expect(describeBlock(block)).toBe('booking.create was declined: declined_by_visitor')
  })
})
